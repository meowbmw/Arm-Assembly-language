...
memcpy_asm: // w0 = d, w1 = s, w2 = n
    ...
    mov x4, x1 // w4 =  w1 = s
    cmp x3, x4 // if d == s
    beq fin_memcpy // return dst
    ble plus_copy // goto plus copy if d<=s
    add x5, x1, w2, uxtw // w5 = s+n, uxtw是无符号扩展,不做的话x和w寄存器无法直接相加,因为位数不同
    cmp x3, x5
    bge plus_copy // goto plus_copy if d>=s+n
    add x3, x3, w2, uxtw // w3=w3+w2=d+n
    add x4, x4, w2, uxtw // w4=w4+w2=s+n
loop1_memcpy:
    cmp w2, #0
    ble fin_memcpy // if n<=0; goto return dst
    sub w2, w2, #1 // --n
    sub x3, x3, #1 // --d
    sub x4, x4, #1 // --s
    ldrb w6, [x4] // int -> register readMemory(4byte) , w6
    strb w6, [x3]
    b loop1_memcpy
plus_copy:
    cmp w2, 0
    ...
    strb w6, [x3]
    sub w2, w2, #1 //--n
    add x3, x3, #1 //++d,因为是对指针做操作,指针就是地址,一定是64位,所以是x
    add x4, x4, #1 //++s
    cmp w2, #0
    bgt plus_copy
fin_memcpy:
